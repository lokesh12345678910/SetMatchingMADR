...
import pandas as pd
import warnings
# Define a custom filter for the specific warning message content
custom_filter = "SettingWithCopyWarning:A value is trying to be set on a copy of a slice from a DataFrame."

# Suppress the specific warning
warnings.filterwarnings("ignore", message=custom_filter)

#DOES NOT OUTPUT ANY WARNINGS
...
def ttest_feature(feature):
  ...
  for i in range(8):
    for j in range(8):
      ...
      if p_value < p_condition:
        print("Within Spanish, for the feature", feature, "p value between set", str(i+1), "and", str(j+1), "is <=", str(p_condition))
        exit(1)
...
def ttest_feature(feature):
  ...
  for i in range(8):
    for j in range(8):
      ...
      _, p_value = stats.ttest_ind(list1, list2)
      if p_value < p_condition:
        print("Within Catalan, for the feature", feature, "p value between set", str(i+1), "and", str(j+1), "is <=", str(p_condition))
        exit(1)
      p_vals.append(p_value)
  return p_vals
...
def ttest_feature(feature):
  ...
  for i in range(8):
    for j in range(8):
      ...
      #assert p_value > p_condition, print("For the feature", feature, "p value between set", str(i+1), "and", str(j+1), "is <=", str(p_condition))
      if p_value < p_condition:
        print("Within English, for the feature", feature, "p value between set", str(i+1), "and", str(j+1), "is <=", str(p_condition))
        exit(1)      
      p_vals.append(p_value)
  return p_vals
...
def ttest_feature_across_lang(featureLang1, featureLang2):
  p_vals = []
  for i in range(8):
    for j in range(8):
      if i == j:
      ...
      _, p_value = stats.ttest_ind(list1, list2)
      if p_value < p_condition:
        print("Across English vs Spanish, for the feature pair", featureLang1, featureLang2, "p value between set", str(i+1), "and", str(j+1), "is <=", str(p_condition))
        exit(1)
      p_vals.append(p_value)
  return p_vals
...
def acrossLanguagePValueEvaluation(p, combination, feat1, feat2):
   if p < p_condition:
      print("For", combination, ",for the feature pair", feat1, feat2, "p value is <=", str(p_condition))
      exit(1)
...
acrossLanguagePValueEvaluation(p_nsyl, "trained Spanish vs trained Catalan", 'Espal_NSYL', 'CAT_SUBTLEX_NSYL')
_, p_nlet = stats.ttest_ind(allTrainedSpanishDF['Espal_NLET'].dropna(), allTrainedCatalanDF['NIM_NLET'].dropna())
acrossLanguagePValueEvaluation(p_nsyl, "trained Spanish vs trained Catalan", 'Espal_NLET', 'NIM_NLET')
...
acrossLanguagePValueEvaluation(p_wordFreq, "trained Spanish vs trained Catalan", 'Coca_word Freq (Total) - Web/Dialects', 'NIM_WordFreq_Relativa')
...
acrossLanguagePValueEvaluation(p_nsyl, "Trained Catalan vs Untrained Catalan", 'CAT_SUBTLEX_NSYL', 'CAT_SUBTLEX_NSYL')
_, p_nlet = stats.ttest_ind(allTrainedCatalanDF['NIM_NLET'].dropna(), allUntrainedDFs['NIM_NLET'].dropna())
acrossLanguagePValueEvaluation(p_nlet, "Trained Catalan vs Untrained Catalan", 'NIM_NLET', 'NIM_NLET')
...
acrossLanguagePValueEvaluation(p_wordFreq, "Trained Catalan vs Untrained Catalan", 'NIM_WordFreq_Relativa', 'NIM_WordFreq_Relativa')
...
acrossLanguagePValueEvaluation(p_nsyl, "Trained Spanish vs Untrained Catalan", 'Espal_NSYL', 'CAT_SUBTLEX_NSYL')
_, p_nlet = stats.ttest_ind(allTrainedSpanishDF['Espal_NLET'].dropna(), allUntrainedDFs['NIM_NLET'].dropna())
acrossLanguagePValueEvaluation(p_nlet, "Trained Spanish vs Untrained Catalan", 'Espal_NSYL', 'NIM_NLET')
...
acrossLanguagePValueEvaluation(p_wordFreq, "Trained Spanish vs Untrained Catalan", 'Coca_word Freq (Total) - Web/Dialects', 'NIM_WordFreq_Relativa')
...
acrossLanguagePValueEvaluation(p_nsyl, "Trained Spanish vs Untrained Spanish", 'Espal_NSYL', 'Espal_NSYL')
_, p_nlet = stats.ttest_ind(allTrainedSpanishDF['Espal_NLET'].dropna(), allUntrainedDFs['Espal_NLET'].dropna())
acrossLanguagePValueEvaluation(p_nlet, "Trained Spanish vs Untrained Spanish", 'Espal_NLET', 'Espal_NLET')
...
acrossLanguagePValueEvaluation(p_wordFreq, "Trained Spanish vs Untrained Spanish", 'Coca_word Freq (Total) - Web/Dialects', 'Coca_word Freq (Total) - Web/Dialects')
...
acrossLanguagePValueEvaluation(p_nsyl, "Trained Catalan vs Untrained Spanish", 'CAT_SUBTLEX_NSYL', 'Espal_NSYL')
_, p_nlet = stats.ttest_ind(allTrainedCatalanDF['NIM_NLET'].dropna(), allUntrainedDFs['Espal_NLET'].dropna())
acrossLanguagePValueEvaluation(p_nlet, "Trained Catalan vs Untrained Spanish", 'CAT_SUBTLEX_NSYL', 'Espal_NSYL')
...
acrossLanguagePValueEvaluation(p_wordFreq , "Trained Catalan vs Untrained Spanish", 'NIM_WordFreq_Relativa', 'Coca_word Freq (Total) - Web/Dialects')
